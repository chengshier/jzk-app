<template>
  <view class="trade-list-page">
    <offline-sale-list v-if="mode==='offlineSale'"/>
    <offline-sale-create v-else-if="mode==='offlineSaleCreate'"/>
    <offline-sale-detail v-else-if="mode==='offlineSaleDetail'" :page-id="id"/>
    <performance-page v-else-if="mode==='performance'"/>
    <operation-profit-page v-else-if="mode==='operationProfit'"/>
    <template v-else>
...
import OfflineSaleList from '@/pages/jk/offlineSale/list.vue';import OfflineSaleCreate from '@/pages/jk/offlineSale/create.vue';import OfflineSaleDetail from '@/pages/jk/offlineSale/detail.vue';import PerformancePage from '@/pages/jk/performance/index.vue';import OperationProfitPage from '@/pages/jk/operationProfit/index.vue';
import { jkBusinessStatusText } from '@/utils/jk-display.js';
const SPECIAL_MODES=['offlineSale','offlineSaleCreate','offlineSaleDetail','performance','operationProfit'];
export default {
  components:{JkPageNav,JkIcon,JkStatusTag,JkEmpty,OfflineSaleList,OfflineSaleCreate,OfflineSaleDetail,PerformancePage,OperationProfitPage},
  data(){return{mode:'order',id:0,status:'',auditFilter:'pending',list:[],loaded:false};},
  computed:{
  ...
  onLoad(query){this.mode=query.mode||'order';this.id=Number(query.id||0);if(!['order','transfer','handleTransfer'].concat(SPECIAL_MODES).includes(this.mode)){this.$util.Tips({title:'无效的业务类型'});return;}if(!SPECIAL_MODES.includes(this.mode))this.loadData();},
