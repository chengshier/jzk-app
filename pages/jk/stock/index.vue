<template>
  <view class="stock-page">
    <jk-page-nav title="我的库存" />
    <view class="stock-content">
      <view class="identity-chip"><jk-icon name="user" size="sm"/><text>{{ stock.identity || roleName || '当前身份' }}身份</text><span>⌄</span></view>
      <view class="summary-card">
        <view><text>库存价值(元)</text><strong>{{ totalValue }}</strong><jk-icon name="money" size="md"/></view>
      ...
      <view class="stock-list">
        <view v-for="item in stockRows" :key="item.key" class="stock-row" @tap="openDetail(item.raw)">
          ...
          <view class="product-main"><text class="product-name">{{ item.name }}</text><text class="sku">{{ item.skuText }}</text><view class="stock-meta"><view><text>当前库存</text><strong :class="{danger:item.empty}">{{ item.availableText }}</strong></view><view><text>参考价</text><strong class="price">{{ item.priceText }}</strong></view></view></view><text class="arrow">›</text>
        </view>
        <jk-empty v-if="loaded && !filteredItems.length" text="暂无库存商品"/>
      </view>
      <button class="flow-btn" @tap="goFlow"><jk-icon name="document" size="sm"/>查看库存流水 <text>›</text></button>
    </view>
  </view>
</template>
<script>
